/* vc4-opc.c -- Table of opcodes for the VC4 processor.
...
#define REG_R(N)    REG(CONCAT2(r,N), N, VC4_R_REGTYPE)
  ...
  REG(r12_L, 12,  VC4_R_REGTYPE),
  REG(r13_L, 13,  VC4_R_REGTYPE),
  REG(ra,    0xe, VC4_R_REGTYPE),
  REG(sp,    0xf, VC4_R_REGTYPE),
  REG(sp_L,  0xf, VC4_R_REGTYPE),
  REG(RA,    0xe, VC4_R_REGTYPE),
};

const reg_entry vc4_regptab[] =
{ /* Build a general purpose register r<N>.  */

#define REG_RP(M,N) REGP((CONCAT2(r,M),CONCAT2(r,N)), CONCAT2(r,N), N, VC4_RP_REGTYPE)
  ...
  REG((r12), 0xc, VC4_RP_REGTYPE),
  REG((r13), 0xd, VC4_RP_REGTYPE),
  //REG((r14), 0xe, VC4_RP_REGTYPE),
  REG((ra), 0xe, VC4_RP_REGTYPE),
  REG((sp), 0xf, VC4_RP_REGTYPE),
};


const unsigned int vc4_num_regs = ARRAY_SIZE (vc4_regtab) ;
const unsigned int vc4_num_regps = ARRAY_SIZE (vc4_regptab) ;

const reg_entry vc4_pregtab[] =
{
/* Build a processor register.  */
  REG(dbs,   0x0, VC4_P_REGTYPE),
  REG(dsr,   0x1, VC4_P_REGTYPE),
  REG(dcrl,  0x2, VC4_P_REGTYPE),
  REG(dcrh,  0x3, VC4_P_REGTYPE),
  REG(car0l, 0x4, VC4_P_REGTYPE),
  REG(car0h, 0x5, VC4_P_REGTYPE),
  REG(car1l, 0x6, VC4_P_REGTYPE),
  REG(car1h, 0x7, VC4_P_REGTYPE),
  REG(cfg,   0x8, VC4_P_REGTYPE),
  REG(psr,   0x9, VC4_P_REGTYPE),
  REG(intbasel, 0xa, VC4_P_REGTYPE),
  REG(intbaseh, 0xb, VC4_P_REGTYPE),
  REG(ispl,  0xc, VC4_P_REGTYPE),
  REG(isph,  0xd, VC4_P_REGTYPE),
  REG(uspl,  0xe, VC4_P_REGTYPE),
  REG(usph,  0xf, VC4_P_REGTYPE),
};

const reg_entry vc4_pregptab[] =
{
  REG(dbs,   0, VC4_P_REGTYPE),
  REG(dsr,   1, VC4_P_REGTYPE),
  REG(dcr,   2, VC4_P_REGTYPE),
  REG(car0,  4, VC4_P_REGTYPE),
  REG(car1,  6, VC4_P_REGTYPE),
  REG(cfg,   8, VC4_P_REGTYPE),
  REG(psr,   9, VC4_P_REGTYPE),
  REG(intbase, 10, VC4_P_REGTYPE),
  REG(isp,   12, VC4_P_REGTYPE),
  REG(usp,   14, VC4_P_REGTYPE),
};

const unsigned int vc4_num_pregs =  ARRAY_SIZE (vc4_pregtab);
const unsigned int vc4_num_pregps =  ARRAY_SIZE (vc4_pregptab);

const char *vc4_b_cond_tab[]=
{
  "eq","ne","cs","cc","hi","ls","gt","le","fs","fc",
  "lo","hs","lt","ge","r", "???"
};

const unsigned int vc4_num_cc =  ARRAY_SIZE (vc4_b_cond_tab);

/* VC4 operands table.  */
const operand_entry vc4_optab[] =
{
...
const unsigned int vc4_num_optab = ARRAY_SIZE (vc4_optab);

/* VC4 traps/interrupts.  */
const trap_entry vc4_traps[] =
{
  {"svc", 5}, {"dvz",  6}, {"flg", 7}, {"bpt", 8}, {"trc", 9},
  {"und", 10}, {"iad", 12}, {"dbg",14}, {"ise",15}
};

const unsigned int vc4_num_traps = ARRAY_SIZE (vc4_traps);

/* VC4 instructions that don't have arguments.  */
const char * vc4_no_op_insn[] =
{
