...
/* VC4 'cinv' options mapping.  */
const cinv_entry vc4_cinvs[] =
{
  {"cinv[i]",     "cinv    [i]"},
  {"cinv[i,u]",   "cinv    [i,u]"},
  {"cinv[d]",     "cinv    [d]"},
...
static int NUMCINVS = ARRAY_SIZE (vc4_cinvs);
...
{
  return (char *) vc4_b_cond_tab[cc_insn];
}
...
getcinvstring (const char *str)
{
  ...
  for (cinv = vc4_cinvs; cinv < (vc4_cinvs + NUMCINVS); cinv++)
...
gettrapstring (unsigned int trap_index)
{
  ...
  for (trap = vc4_traps; trap < vc4_traps + NUMTRAPS; trap++)
...
getregname (reg r)
{
  const reg_entry * regentry = vc4_regtab + r;

  if (regentry->type != VC4_R_REGTYPE)
    return ILLEGAL;
...
getregpname (reg r)
{
  const reg_entry * regentry = vc4_regptab + r;

  if (regentry->type != VC4_RP_REGTYPE)
    return ILLEGAL;
...
getprocregname (int reg_index)
{
  ...
  for (r = vc4_pregtab; r < vc4_pregtab + NUMPREGS; r++)
...
getprocpregname (int reg_index)
{
  ...
  for (r = vc4_pregptab; r < vc4_pregptab + NUMPREGPS; r++)
    if (r->image == reg_index)
      return r->name;
...
match_opcode (void)
{
  ...
  instruction = &vc4_instruction[NUMOPCODES - 2];

  /* Loop over instruction table until a full match is found.  */
  while (instruction >= vc4_instruction)
    {
      mask = build_mask ();
      /* Adjust mask for bcond with 32-bit size instruction */
      if ((IS_INSN_MNEMONIC("b") && instruction->size == 2))
...
print_arg (argument *a, bfd_vma memaddr, struct disassemble_info *info)
{
  ...
  switch (a->type)
      ...
      if (IS_INSN_TYPE (BRANCH_INS) || IS_INSN_MNEMONIC ("bal"))
	{
	  relative = 1;
	  longdisp = a->constant;
	  /* REVISIT: To sync with WinIDEA and VC4 4.1tools, the below
	     line commented */
	  /* longdisp <<= 1; */
	  mask = ((LONGLONG)1 << a->size) - 1;
	  switch (a->size)
...
int vc4_decode(const struct vc4_info *info, bfd_vma memaddr, struct disassemble_info *dis_info);
...
int vc4_decode(const struct vc4_info *info, bfd_vma memaddr, struct disassemble_info *dis_info)
{
  ...
  for (i = 0; i < 5; i++) {
    status = dis_info->read_memory_func(memaddr + i * 2, buffer + i * 2, 2, dis_info);
    if (status != 0)
      break;
  }
  len = i * 2;

  const struct vc4_opcode *op = vc4_get_opcode(info, buffer, len);
  assert(op != NULL);

  ll = vc4_display(info, op, memaddr, buffer, len);
...
print_insn_vc4 (bfd_vma memaddr, struct disassemble_info *info)
{
  ...
  if (vc4_info != NULL) {
    return vc4_decode(vc4_info, memaddr, info);
  }
  return 2;
#else
